...
export default async function ProductsPage() {
  const performance = await mockBusinessRepository.getProductPerformance();
  ...
  return (
    <main className="min-h-screen bg-white px-6 py-10 text-neutral-900 dark:bg-neutral-950 dark:text-neutral-100 md:px-12">
      <div className="mx-auto max-w-5xl">
        ...
        <p className="mt-2 max-w-2xl text-neutral-600 dark:text-neutral-400">Demo Business Data — Toko Sejahtera.</p>
          ...
          <table className="w-full min-w-[560px] text-sm">
            <thead>
              <tr className="border-b border-neutral-200 text-left text-xs uppercase tracking-wide text-neutral-500 dark:border-neutral-800">
                ...
                <th className="py-2 pr-4 font-medium">Omzet</th>
                <th className="py-2 pr-4 font-medium">Unit</th>
                <th className="py-2 pr-4 font-medium">Growth</th>
                <th className="py-2 font-medium">Kontribusi</th>
              </tr>
            </thead>
            <tbody>
              {performance.map((p) => {
                const up = p.growthPercentage >= 0;
                return (
                  <tr key={p.productId} className="border-b border-neutral-100 last:border-0 dark:border-neutral-900">
                    ...
                    <td className="py-3 pr-4">
                      ...
                      >
                        ...
                        {p.growthPercentage.toFixed(1)}%
                      </span>
                    </td>
                    <td className="py-3 tabular-nums text-neutral-700 dark:text-neutral-300">
                      {p.contributionPercentage.toFixed(1)}%
                    </td>
                  </tr>
                );
              })}
