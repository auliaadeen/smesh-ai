...
export class SupabaseBusinessRepository implements BusinessRepository {
  ...
  async getSalesComparison(): Promise<SalesComparison> {
    ...
    const previousSales = sales.filter(
      (sale) => sale.soldAt === comparisonDate
    );
    ...
    return inventory
      ...
      .map((inv) => {
        ...
        return {
          ...
          salesVelocity: calculateSalesVelocity(
            sumUnits(recentSales),
      ...
      .sort((a, b) => {
        ...
        return (
          rank[a.urgency] - rank[b.urgency] || a.currentStock - b.currentStock
        );
      });
  }
  ...
  async getProductPerformance(
    ...
    const currentSales = sales.filter(
      (sale) => sale.soldAt >= currentStart && sale.soldAt <= today
    );

    const previousSales = sales.filter(
      (sale) => sale.soldAt >= previousStart && sale.soldAt <= previousEnd
    );
    ...
    return productId
      ? performance.filter((item) => item.productId === productId)
      : performance;
  }
}
